from ai_mv.core.stages.plan_citypop_mv import build_plan_preview_payload, _merged_shot_section_type


def test_citypop_plan_uses_audio_sections_and_stays_within_m1_bounds():
    payload = {
...
def test_citypop_plan_splits_long_sections_for_m1():
    payload = {
...
def test_citypop_plan_assigns_progressive_variants_within_long_section():
...
def test_citypop_plan_falls_back_without_audio_sections():
    out = build_plan_preview_payload(
        {},
        {
            "concept_text": "Japanese 80s city pop summer dusk",
...
def test_citypop_plan_builds_rich_render_prompts():
    out = build_plan_preview_payload(
        {},
        {
...
def test_citypop_plan_uses_prechorus_progression_hint_before_chorus_hint():
    out = build_plan_preview_payload(
        {},
        {
            "concept_text": "Japanese 80s city pop night drive",
...
def test_citypop_plan_can_route_ia2v_for_chorus_when_enabled():
    out = build_plan_preview_payload(
        {
            "planning": {
                "enable_ia2v": True,
...
def test_citypop_plan_keeps_i2v_when_ia2v_is_disabled():
    out = build_plan_preview_payload(
...
def test_citypop_plan_can_route_flf2v_for_bridge_when_enabled():
    out = build_plan_preview_payload(
        {
            "planning": {
                "enable_flf2v": True,
...
def test_citypop_plan_does_not_drop_tail_when_shot_count_exceeds_m1_max():
    out = build_plan_preview_payload(
        {},
        {
            "concept_text": "Japanese 80s city pop night drive",
...
def test_citypop_plan_sorts_and_clamps_sections_before_building_shots():
    out = build_plan_preview_payload(
        {},
        {
            "concept_text": "Japanese 80s city pop night drive",
...
def test_citypop_plan_prefers_more_prominent_section_type_when_merging_across_boundary():
    assert _merged_shot_section_type({"section_type": "verse"}, {"section_type": "pre_chorus"}) == "pre_chorus"
    assert _merged_shot_section_type({"section_type": "pre_chorus"}, {"section_type": "chorus"}) == "chorus"
