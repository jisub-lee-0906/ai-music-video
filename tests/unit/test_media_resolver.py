...
def test_build_merge_plan_uses_ref_anchor_timeline():
    payload = {
        "clips": [
        ...
        "prompt_plan": {
            "ref_items": [
                {"shot_id": "s1", "start_sec": 8.0},
                {"shot_id": "s2", "start_sec": 12.0},
                {"shot_id": "s3", "start_sec": 16.0},
            ],
            "wan_items": [
    ...
    assert out["ordered"][0] == {"kind": "still", "image": "ref1.png", "duration_sec": 8.0, "label": "head_hold"}
    assert out["ordered"][1] == {"kind": "video", "path": "clip_2.mp4", "label": "s2"}
    assert out["ordered"][2] == {"kind": "video", "path": "clip_3.mp4", "label": "s3"}
    assert out["ordered"][3] == {"kind": "still", "image": "ref3.png", "duration_sec": 14.0, "label": "tail_hold"}
