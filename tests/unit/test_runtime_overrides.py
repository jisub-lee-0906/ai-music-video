...
def test_apply_runtime_overrides_rewrites_windows_paths_when_env_present(monkeypatch):
    cfg = {
        "integrations": {
            "comfyui_base_url": "http://127.0.0.1:8000",
            "comfyui_input_dir": r"C:\Users\Desktop\Documents\ComfyUI\input",
...
def test_apply_runtime_overrides_uses_detected_defaults_when_env_missing(monkeypatch):
    cfg = {
        "integrations": {
            "comfyui_base_url": "http://127.0.0.1:8000",
            "comfyui_input_dir": r"C:\Users\Desktop\Documents\ComfyUI\input",
    ...
    monkeypatch.setattr(
        "ai_mv.core.orchestration.runtime_overrides._default_comfy_output_dir",
        lambda: r"C:\\Users\\Desktop\\Documents\\ComfyUI\\output",
    )
    monkeypatch.setattr(
        "ai_mv.core.orchestration.runtime_overrides._default_codex_bin",
...
def test_discover_comfy_dir_finds_user_documents_path(tmp_path):
    ...
    assert _discover_comfy_dir("input", users_root) == str(users_root / "Alice" / "Documents" / "ComfyUI" / "input")
    assert _discover_comfy_dir("output", users_root) == str(users_root / "Alice" / "Documents" / "ComfyUI" / "output")



def test_discover_comfy_dir_skips_inaccessible_candidate_and_prefers_accessible_one(monkeypatch, tmp_path):
    users_root = tmp_path / "Users"
    ...
    assert _discover_comfy_dir("input", users_root) == str(desktop)



def test_default_codex_bin_uses_home_fallback(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("ai_mv.core.orchestration.runtime_overrides.shutil.which", lambda _name: None)
