...
def test_verbalize_ref_prompt_pairs_uses_minimal_place_action_carry():
    ...
    assert out["S001"]["start_prompt_text"].startswith("The woman is writing in a worn notebook in a dim late-night diner.")
...
def test_verbalize_wan_prompts_uses_compact_bridge_sentence():
    ...
    assert out["S002"].startswith("The woman is crossing the rainy street with one readable step forward on a wet city street at night.")
