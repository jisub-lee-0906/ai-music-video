...
def test_pipeline_runs_ordered_stages(monkeypatch):
    ...
    monkeypatch.setattr(pipeline, "run_lyrics_timeline", _stage("lyrics_timeline", {"lyrics_timeline": {"sections": [{"section_name": "Verse 1", "lyric_beats": [{"beat_id": "b1", "start_sec": 0.0, "end_sec": 2.0}]}]}}))
    monkeypatch.setattr(pipeline, "run_scene_plan", _stage("scene_outline", {"scene_outline": {"shot_packages": [{"shot_id": "b1"}]}}))
    monkeypatch.setattr(pipeline, "run_director_plan", _stage("direction_plan", {"direction_plan": {"shot_packages": [{"shot_id": "b1"}]}}))
    monkeypatch.setattr(pipeline, "run_render_plan", _stage("prompt_plan", {"prompt_plan": {"ref_items": [{"shot_id": "b1"}], "wan_items": []}}))
