...
def test_pipeline_promotes_stage_assembly_revision_into_canonical_review_report(monkeypatch):
    final_payload: dict = {}
    ...
    monkeypatch.setattr(
        ...
        "init_run_state",
        lambda cfg, run_id, allow_existing=False: {
            "run_id": run_id or "run-rerender-assembly-revision",
            "status": "running",
            "current_stage": "",
            "failure_reason": "",
            "completed_stages": [],
    ...
    monkeypatch.setattr(
        ...
        lambda stage_input: StageOutput(
            ...
            "done",
            {
                ...
                "rerender_final_video": "final-revised.mp4",
    ...
    monkeypatch.setattr(
        ...
        lambda stage_input: StageOutput("rerender_escalation", "done", {"rerender_escalation": {"status": "manual_review_required"}}),
    )

    pipeline.run_pipeline({"concept_text": "citypop night drive"}, "assembly-revision-publish-test", allow_existing_run=True)

    assert final_payload["assembly_revision_result"]["status"] == "applied"
    assert final_payload["final_video"] == "final-revised.mp4"
    assert final_payload["review_report"]["assembly_revision_summary"] == {
        "present": True,
        "action": "revise_assembly_coverage_before_sync_pad",
        "target": "assembly",
...
def test_pipeline_marks_unresolved_rerender_outcome_when_loop_still_needs_rerender(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda stage_input: StageOutput(
            "rerender_escalation",
            "done",
            {"rerender_escalation": {"status": "manual_review_required", "shot_ids": ["S001"], "shot_count": 1, "summary_by_shot": [{"shot_id": "S001", "reason_codes": ["continuity_break"], "priority_score": 7, "recommended_action": "rerender_continuity_break_shots", "rerender_prescription": {"stage_focus": "review", "workflow_focus": None, "prompt_contract_focus": [], "fix_strategy": "inspect_review_failures_manually"}, "packet_artifacts": {"review_packet_manifest": "review-packet.json"}, "reviewer_note": "Inspect shot S001 in the review packet artifacts (reasons: continuity_break)"}], "video_path": "final.mp4", "reviewer_summary": "Manual review required for 1 shots: S001", "artifacts": {"review_packet_manifest": "review-packet.json"}}},
        ),
    )
        ...
        "summary_by_shot": [
            {
                ...
                "rerender_prescription": {"stage_focus": "review", "workflow_focus": None, "prompt_contract_focus": [], "fix_strategy": "inspect_review_failures_manually"},
