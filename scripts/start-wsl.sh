...
usage() {
  cat <<'EOF'
Usage: ./scripts/start-wsl.sh [--run-id ID] [--concept-text TEXT]
...
if ! grep -qi microsoft /proc/version 2>/dev/null && [ ! -e /proc/sys/fs/binfmt_misc/WSLInterop ]; then
  echo "error: start-wsl.sh must be run inside WSL" >&2
  exit 1
fi
...
echo "[start-wsl] repo=$REPO_ROOT"
echo "[start-wsl] comfyui_base_url=http://${GATEWAY_HOST}:8000"
echo "[start-wsl] comfyui_input_dir=$COMFY_INPUT_DIR"
echo "[start-wsl] comfyui_output_dir=$COMFY_OUTPUT_DIR"
echo "[start-wsl] codex_cli_path=$CODEX_BIN"
...
from ai_mv.entrypoints import start as start_entry
...
start_entry.default_config = _wsl_default_config
start_entry.apply_defaults = _wsl_apply_defaults

raise SystemExit(run_start(os.environ['AI_MV_RUN_ID'] or None, os.environ['AI_MV_CONCEPT_TEXT'] or None))
PY
