...
usage() {
  cat <<'EOF'
Usage: ./scripts/preflight-wsl.sh [--run-id ID] [--concept-text TEXT] [--full-run]
...
ai_mv_print_wsl_env "preflight-wsl"
[ "$SMOKE_MODE" = "1" ] && echo "[preflight-wsl] mode=smoke (15-20s, ia2v/flf2v disabled)"
[ "$SMOKE_MODE" = "0" ] && echo "[preflight-wsl] mode=full-run"
[ -n "$RUN_ID" ] && echo "[preflight-wsl] run_id=$RUN_ID"
[ -n "$CONCEPT_TEXT" ] && echo "[preflight-wsl] concept_text=$CONCEPT_TEXT"

CMD=("$AI_MV_PYTHON_BIN" -m ai_mv.cli.app preflight)
