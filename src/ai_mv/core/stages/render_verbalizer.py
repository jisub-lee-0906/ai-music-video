...
def _verbalize_ref_prompt_pairs_with_codex(config: dict, rows: list[dict]) -> dict[str, dict]:
    ...
    prompt = (
        ...
        "The planner may also provide a hidden blocking contract with blocking_role, entry_side, travel_axis, frame_bias, arrival_side, and camera_relation; preserve that staging difference through physical space wording instead of turning every shot into the same centered walk. "
        ...
        "If the blocking contract implies edge_entry, side_handoff, walk_away, middle-right carry, road-side re-entry, or compressed hold, keep that staging readable through physical location wording such as left edge, right edge, road-side edge, middle-right side, walks away, keeps close, or far side, but do not mention frames or cameras directly. "
        ...
        "For each shot, produce start_prompt_text and end_prompt_text by naturally merging: subject_intro, dominant_action, location or primary_surface, blocking/staging implication when it materially changes the shot, state/action, story_event when it adds concrete beat-level change, support_detail only if still secondary, and lighting. "
...
def _verbalize_wan_prompts_with_codex(config: dict, rows: list[dict]) -> dict[str, str]:
    ...
    prompt = (
        ...
        "The planner may also provide a hidden blocking contract with blocking_role, entry_side, travel_axis, frame_bias, arrival_side, and camera_relation; preserve that staging difference through physical route wording instead of flattening every bridge into the same centered carry. "
