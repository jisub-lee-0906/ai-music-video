...
from ai_mv.engines.scene_plan.planner import build_scene_plan, build_scene_plan_preview_prompt


def run_scene_plan(stage_input: StageInput) -> StageOutput:
    plan = build_scene_plan(stage_input.config, stage_input.payload)
    return StageOutput("scene_plan", "done", _build_payload(stage_input.payload, plan, stage_input.config), [])


def build_scene_plan_preview_payload(config: dict, payload: dict) -> dict:
    plan = build_scene_plan(config, payload)
...
def _build_payload(payload: dict, plan: dict, config: dict) -> dict:
    ...
    return {
        ...
        "planner_prompts": merge_planner_prompt(
            payload,
            "scene_plan",
            {"prompt": build_scene_plan_preview_prompt(config, payload)},
        ),
    }
