...
def _ensure_lyrics(config: dict) -> None:
    ...
    if path.exists() and path.read_text(encoding="utf-8").strip():
        return
    prompt = "Create concise K-pop MV lyrics under 160 seconds. Return JSON {\"lyrics\": \"...\"}."
    schema = {"type": "object", "required": ["lyrics"], "properties": {"lyrics": {"type": "string"}}}
    out = generate_structured(config, prompt, schema)
    lyrics = str(out.get("lyrics", "Instrumental intro, cinematic chorus.")).strip()
...
def _ensure_run_style(config: dict, run_dir: Path) -> None:
    ...
    if style_file.exists():
        return
    prompt = "Return JSON {\"guidance\":\"...\"} for cinematic live-action music video style."
    schema = {"type": "object", "required": ["guidance"], "properties": {"guidance": {"type": "string"}}}
    out = generate_structured(config, prompt, schema)
    guidance = str(out.get("guidance", "cinematic live-action, dynamic camera, coherent character identity"))
