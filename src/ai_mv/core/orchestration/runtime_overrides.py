...
def apply_runtime_overrides(config: dict) -> dict:
    ...
    comfy_host = str(os.getenv("AI_MV_COMFY_HOST") or "").strip()
    ...
    elif comfy_host and _needs_base_url_override(base_url):
        integrations["comfyui_base_url"] = f"http://{comfy_host}:8000"
...
def _is_linux_path(raw: str) -> bool:
    value = raw.strip()
...
def _default_codex_bin() -> str | None:
    home_dir = Path(str(os.getenv("HOME") or "").strip() or Path.home())
    home_fallback = str(home_dir / ".hermes" / "node" / "bin" / "codex").strip()
    for raw in (
