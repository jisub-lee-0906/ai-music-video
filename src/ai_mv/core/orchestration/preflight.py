...
from ai_mv.core.stages.storyboard import build_storyboard_preview_payload
...
def run_preflight(config: dict, run_id: str = "", allow_existing_run: bool = False) -> str:
    ...
    try:
        ...
        _run_stage(state, stage_input, "storyboard", _add_storyboard)
...
def _add_storyboard(stage_input: StageInput) -> None:
    stage_input.payload.update(build_storyboard_preview_payload(stage_input.config, stage_input.payload))
