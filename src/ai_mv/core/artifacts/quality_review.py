...
def write_quality_review(state: dict, review: dict) -> None:
    ...
    write_json(run_file(state["run_id"], "quality_review.json", scope), review)
    write_json(latest_file("quality_review.json", scope), review)
    if str(state.get("status", "")) == "done":
        write_json(latest_success_file("quality_review.json", scope), review)
