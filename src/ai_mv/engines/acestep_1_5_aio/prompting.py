...
def _audio_songform_rules(plan: dict) -> str:
    ...
    if str(plan.get("songform_mode", "")).strip().lower() == "hook_validation":
        return (
            ...
            + variant_text
            + "Do not include Intro, Verse 1, or Pre-Chorus in a thirty-second validation take; ACE-Step often collapses them into the hook at this length. "
            "Do not force Verse 2, Bridge, or Final Chorus into a thirty-second validation take. "
            "Make this an original-B-like Chorus -> Outro proof of genre, vocal, hook, and clean ending. "
            + _hook_validation_timing_rules(plan)
            + "Use exactly three Chorus lyric lines: line one should be the title-grade hook, line two should answer it simply, and line three should repeat or lightly vary the title-grade hook. "
            "Keep all Chorus lines as one-breath phrases, six words or fewer, with simple open vowels. "
            "Avoid cramming syllables, long clauses, stacked images, or fast internal rhyme into Chorus or Outro lines. "
            "Use exactly two Outro lyric lines as a repeated closing tag, preferably the same short phrase twice. "
            "Keep the Outro tag tender and minimal, like a clean last breath. "
            "Outro tag must feel like the last sung breath, not a new section. "
            "Do not add a fourth Chorus narrative payoff line; late new story lines can make ACE-Step jump into a different chorus idea. "
            "Keep any Intro omitted or effectively instantaneous. "
