...
def _planner_prompt(config: dict, payload: dict, clips: list[dict], carry: str) -> str:
    ...
    return (
        ...
        "Do not mention character appearance, outfit colors, hair color, rendering style, cel shading, outlines, or background design style in the positive prompt. "
        "A short motion-style reminder such as 'in stable 2d anime motion' is allowed only inside the camera sentence, but do not turn it into a style paragraph. "
        ...
        "The background sentence should describe what an already-visible element does, such as a gate opening, lights flickering, window reflections sliding, puddle rings widening, or signs rotating. "
        "Do not invent new objects, new vehicles, new people, or new scenery that are not already implied by the source frames. "
        ...
        "Do not begin the third sentence with 'Background the' or 'Background a'. Start with a plural or mass noun phrase instead, such as 'Background neon lights...', 'Background window reflections...', 'Background puddle rings...', or 'Background reflected light...'. "
        "Write patterns such as 'Background clock hands tick while reflected light trembles on the floor' or 'Background neon lights flicker rapidly'. "
        "Good background sentence examples: 'Background neon lights flicker rapidly', 'Background window reflections slide across the glass', 'Background puddle rings widen under each step', or 'Background gate arms snap back in sequence'. "
        ...
        "Prefer plural or mass nouns after 'Background' such as lights, reflections, rings, signs, gate arms, puddle ripples, or window streaks. "
        ...
        "Keep the background sentence eventful and specific. Prefer window reflections sliding, sign lights flickering, puddle rings widening, gate arms snapping back, reflected light shifting, or UI rings rotating. "
        "If the source beat is low-energy, the background sentence should still describe a subtle physical change, such as reflected light drifting, floor reflections trembling, or window streaks narrowing. "
        "Favor macro or reactive motion over newly staged action. Safe examples are rain falling, fog rolling, light flickering, reflections sliding, hair fluttering, or jacket hems moving in the wind. "
        "Avoid sequencing brand-new events such as cars passing, birds appearing, crowds entering, or signs multiplying. "
        ...
        "Write items like '3d render', 'photorealistic', 'cgi', 'warped hands', 'extra limbs', 'static', 'slow motion', 'realistic human physics', or 'smooth transitions'. "
        ...
        "Example positive_prompt: 'Camera shakes violently on impact in stable 2d anime motion. The girl swings the guitar down with extreme force. Background neon lights flicker rapidly.' "
        "Another valid example: 'Camera is completely locked off in stable 2d anime motion. The girl darts her eyes sharply to the left. Background holographic UI rings rotate clockwise.' "
        "Example negative_prompt: '3d render, photorealistic, volumetric lighting, soft shading, morphing, melting guitar, warping limbs, static, slow motion, realistic human physics'. "
