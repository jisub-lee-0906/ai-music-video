...
def _rewrite_with_codex(config: dict, rows: list[dict]) -> dict[str, dict]:
    ...
    prompt = (
        ...
        "For final-opening shots, do not make smiling by a window, facing a reflection, lingering under glow, following warm light ahead, pale dawn light, walking toward a vague opening, reflections brightening behind her, lifting her face to a sign, opening her fingers toward a clearing path, or settling/steadying motion the main event when a forward crossing action is possible in the same place. "
        "For final-opening shots, keep the destination physical and immediate: gate, threshold, doorway, exit line, stair top, or platform edge, not a distant bright point or symbolic sign. "
        ...
        "Avoid generic release wording such as one steady rhythm, open space, brighter direction of travel, or path clearing when a specific crossing action exists in the same place. "
