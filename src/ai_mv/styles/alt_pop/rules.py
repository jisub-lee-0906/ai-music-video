...
def alt_pop_section_shot_specs(section_type: str, duration_sec: float) -> list[dict]:
    ...
    if section_type == "intro":
        return [_spec("intro_edge", "high_edge", "low", render_mode, 1.0)]
    ...
    if section_type == "bridge":
        return [_spec("transparent_bridge", "transparent_bridge", "medium", render_mode, 1.0)]
    if section_type == "outro":
        return [_spec("outro_release", "release_stride", "low", render_mode, 1.0)]
    return [_spec("verse_edge", "transparent_passage", "medium", render_mode, 1.0)]
